...
# For parsing the forced JSON output
from langchain_core.output_parsers import JsonOutputParser

# --- 1. Define Citation Schema (Pydantic) ---

# NOTE: The schema is correct. It's a blueprint for the output we want.
...
# We'll use ChatGroq as you specified in your code, which is a great choice.
# Make sure your GROQ_API_KEY and HUGGINGFACEHUB_API_TOKEN are set in your .env file.
LLM_REPO_ID = "llama-3.3-70b-versatile"
EMBEDDING_MODEL_NAME = "BAAI/bge-small-en-v1.5"

# Global variables
VECTOR_STORE = None
ALL_CHUNKS_FOR_LOOKUP = [] # Store all chunks with metadata for citation lookup
...
def create_rag_chain():
    ...
    # conforming to the required schema. We use `get_format_instructions`
    # to get a detailed prompt from the 
    """Invokes the RAG chain in streaming mode and formats the output."""

    # We will build the final answer and collect citations
    final_answer = ""
    all_cited_ids = []
    
    # 1. Stream the response from the LLM
    print("💡 **Answer**:")
    try:
        # The chain now streams an object that is then parsed by the output parser.
        # We need to loop through the streamed chunks.
        for chunk in rag_chain.stream(question):
            # Each chunk is a part of the `CitedAnswer` object.
            # We can process the 'answer' part as it comes in.
            if hasattr(chunk, 'answer'):
                answer_chunk = chunk.answer
                final_answer += answer_chunk
                print(answer_chunk, end="", flush=True)

            # We also collect all citations as they appear in the stream.
            if hasattr(chunk, 'citations'):
                all_cited_ids.extend(chunk.citations)
    parser.
    format_instructions = output_parser.get_format_instructions()
    
    # This is the new, robust prompt
    system_prompt = (
    ...
    ).partial(format_instructions=format_instructions) # Use partial to inject the instructions

    # LCEL Chain
    # The `output_parser` is now part of the chain, automatically handling the JSON conversion.
    rag_chain = (
...
def query_pdf(question: str, rag_chain: Any) -> Dict[str, Any]:            
        ...
        print(f"\n\nError in structured response: {e}")
        return {"answer": final_answer, "citations": []}

    # Add a newline for clean output after the streaming is complete
    print("\n")
    ...
    unique_cited_ids = sorted(list(set(all_cited_ids)))
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(result['answer'])
        if result['citations']:
         print("--- Citations (Source Details) ---")
